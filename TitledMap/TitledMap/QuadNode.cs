...
namespace TitledMap
{
    class QuadNode
    {
        ...
        public void buildQuadNode(List<GameObject> listObject)
        {
            ...
            {
                _nodeTL = new QuadNode(this._id * 10 + 1, new Rectangle(this._bounds.X, this._bounds.Y, this._bounds.Width / 2, this._bounds.Height / 2));
                _nodeTR = new QuadNode(this._id * 10 + 2, new Rectangle(this._bounds.X + this._bounds.Width / 2, this._bounds.Y, this._bounds.Width / 2, this._bounds.Height / 2));
                _nodeBL = new QuadNode(this._id * 10 + 3, new Rectangle(this._bounds.X, this._bounds.Y + this._bounds.Height / 2, this._bounds.Width / 2, this._bounds.Height / 2));
                _nodeBR = new QuadNode(this._id * 10 + 4, new Rectangle(this._bounds.X + this._bounds.Width / 2, this._bounds.Y + this._bounds.Height / 2, this._bounds.Width / 2, this._bounds.Height / 2));
        ...
        public void clip(List<GameObject> listObject, QuadNode node)
        {
            ...
            {
                int temp = Rectangle.Intersect(objectItem._bounds, node._bounds).Height;
